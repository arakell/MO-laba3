...
public:
	...
	SecondPirson(Book* book, Sheet* sheet, Format* format, vector<double> X, Function f, Gradient grad) {
	...
	void method() {
		...
		do {
			...
			double m = deltaX[0] * deltaG[0] + deltaX[1] * deltaG[1];
			cout << m << endl;

			N[0][0] += temp[0] * deltaX[0] / m;
			N[0][1] += temp[0] * deltaX[1] / m;
			N[1][0] += temp[1] * deltaX[0] / m;
			N[1][1] += temp[1] * deltaX[1] / m;
